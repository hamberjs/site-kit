...
describe('slug', () => {
	describe('urlsafeSlugProcessor', () => {
		describe('ascii', () => {
			...
			it('punctuated text', () => {
				assert.equal(
					urlsafeSlugProcessor('hamber.VERSION'),
					`hamber${_}VERSION`
				);
			});
			...
			it('text containing the colon char', () => {
				assert.equal(
					urlsafeSlugProcessor('hamber:body'),
					`hamber${_}body`
				);
			});
			it('text containing the slash char', () => {
				assert.equal(
					urlsafeSlugProcessor('hamber/motion'),
					`hamber${_}motion`
				);
			});
		...
		describe('ascii + cyricllic', () => {
			...
			it('text containing the colon char', () => {
				assert.equal(
					urlsafeSlugProcessor('hamber:body Всплытие и:перехват событий'),
					`hamber${_}body${_}Vsplytie${_}i${_}perehvat${_}sobytij`
				);
			});
			it('text containing the slash char', () => {
				assert.equal(
					urlsafeSlugProcessor('hamber/motion Всплытие и / перехват/событий'),
					`hamber${_}motion${_}Vsplytie${_}i${_}perehvat${_}sobytij`
				);
			});
			it('text containing the comma char', () => {
				assert.equal(
	...
	describe('unicodeSafeProcessor (preserve unicode)', () => {
		describe('ascii', () => {
			...
			it('punctuated text', () => {
				assert.equal(
					unicodeSafeProcessor('hamber.VERSION'),
					`hamber${_}VERSION`
				);
			});
			it('text starting with the dollar sign', () => {
			...
			it('text containing the colon char', () => {
				assert.equal(
					unicodeSafeProcessor('hamber:body'),
					`hamber${_}body`
				);
			});
			it('text containing the slash char', () => {
				assert.equal(
					unicodeSafeProcessor('hamber/motion'),
					`hamber${_}motion`
				);
			});
			it('text containing the comma char', () => {
		...
		describe('ascii + cyricllic', () => {
			...
			it('text containing the colon char', () => {
				assert.equal(
					unicodeSafeProcessor('hamber:body Всплытие и:перехват событий'),
					`hamber${_}body${_}Всплытие${_}и${_}перехват${_}событий`
				);
			});
			it('text containing the slash char', () => {
				assert.equal(
					unicodeSafeProcessor('hamber/motion Всплытие и / перехват/событий'),
					`hamber${_}motion${_}Всплытие${_}и${_}перехват${_}событий`
				);
			});
			it('text containing the comma char', () => {
				assert.equal(
